...
if __name__ == "__main__":
    ...
    mesa = int(sys.argv[5])
    ruta_imagen_a_buscar = sys.argv[6]
    ruta_imagen_dealer = sys.argv[7]
    ...
    if ubicacion_primera is not None:
        ...
        #print(f"Ubicacion time: {ubicacion_primera}")


        # Definir las nuevas imágenes a buscar y sus regiones relativas a la primera imagen encontrada
        nuevas_imagenes_a_buscar = ruta_imagen_dealer
        nuevas_regiones = [
            (top, left, width, height),  # Región relativa a la primera imagen
        ]

        for i, (ruta_imagen, region) in enumerate(zip(nuevas_imagenes_a_buscar, nuevas_regiones)):
            thread = threading.Thread(target=encontrar_imagen, args=(ruta_imagen, region, result_queue, f"image_{i+2}"))
        ...
        while not result_queue.empty():
            key, ubicacion = result_queue.get()
            ubicaciones[key] = ubicacion
            #print(f"{key}: {ubicacion}")
            x, y = ubicacion
            #print(x)
            # Imprimir solo la coordenada x después de haber obtenido todas las ubicaciones
            #print(x_primera)
            #print(x_primera-x)
            for key, ubicacion in ubicaciones.items():
                x, y = ubicacion  # Descomponer la tupla y asignar solo la coordenada x
             ...
             # Evaluar la condición sobre x
                if 100 < (x_primera - x) < 300:
                    #print("entramos")
                    P1_dealer = 1
                    P2_dealer = 0
                    P3_dealer = 1
                elif 0 < (x_primera - x) < 100:
                    P1_dealer = 0
                    P2_dealer = 1
                    P3_dealer = 1
                elif (x_primera - x) < 0:
            ...
            #print(f"{key} valores asignados: P1_dealer = {P1_dealer}, P2_dealer = {P2_dealer}, P3_dealer = {P3_dealer}")90-23
        ...
        regions = [
            ((x_primera - 300, y_primera - 292, 90, 16), 200, '', '--psm 7', 'P2_name'), 
            ((x_primera - 300, y_primera - 273, 70, 18), 200, '0123456789.', '--psm 7 -c tessedit_char_whitelist=0123456789.', 'P2_stack'),
            ((x_primera - 200, y_primera - 250, 50, 16), 200, '0123456789.', '--psm 7 -c tessedit_char_whitelist=0123456789.', 'P2_bet'),
            ((x_primera + 265, y_primera - 292, 90, 16), 200, '', '--psm 7', 'P3_name'),
            ((x_primera + 265, y_primera - 273, 70, 18), 200, '0123456789.', '--psm 7 -c tessedit_char_whitelist=0123456789.', 'P3_stack'),
            ((x_primera + 205, y_primera - 250, 50, 16), 200, '0123456789.', '--psm 7 -c tessedit_char_whitelist=0123456789.', 'P3_bet'),
            ((x_primera - 10, y_primera - 270, 90, 23), 200, '', '--psm 7', 'bote_grande'),
            ((x_primera + 0, y_primera - 170, 50, 40), 200, '0123456789.', '--psm 7 -c tessedit_char_whitelist=0123456789.', 'bote'),
            ((x_primera - 105, y_primera + 0, 72, 42), 100, '0123456789,.', '--psm 7', 'stackefectivo'),
            ((x_primera + 30, y_primera - 96, 50, 20), 200, '', '--psm 6 -c tessedit_char_whitelist=0123456789dhsc', 'p1bet'), 
            #((x_primera + 495, y_primera + 145, 20, 35), 200, '', '--psm 6 -c tessedit_char_whitelist=23456789AJKQT', 'carta1'),
            #((x_primera + 547, y_primera + 145, 20, 35), 200, '', '--psm 7 -c tessedit_char_whitelist=23456789AJKQT', 'carta2'),  
        ]
